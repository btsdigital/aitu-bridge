...
describe('Web Bridge', () => {
  ...
  it('should get item from storage', async () => {
    ...
    expect(webBridge.execute).toHaveBeenNthCalledWith(1, 'storage', 'storage:1', 'getItem', { keyName: 'testKey' });
    expect(webBridge.execute).toHaveBeenNthCalledWith(2, 'storage', 'storage:2', 'getItem', { keyName: 'testKey2' });
  ...
  it('should set item to storage', async () => {
    webBridge.execute.mockResponseOnce({ reqId: `storage:1`, data: 'success', error: null } as const, { delay: 250 });

    const resultA = aituBridge.storage.setItem('key1', 'value1');

    vi.advanceTimersByTime(250);

    expect(webBridge.execute).toHaveBeenNthCalledWith(1, 'storage', 'storage:1', 'setItem', { keyName: 'key1', keyValue: 'value1' });
  ...
  it('should clear storage', async () => {
    ...
    expect(webBridge.execute).toHaveBeenCalledWith('storage', 'storage:1', 'clear', {});
    await expect(result).resolves.toStrictEqual('success');
